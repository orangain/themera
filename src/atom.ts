...
export const chakraColorAtom = atom<ThemeOverride["colors"]>((get) => {
  ...
  Object.entries(overrideConfig).forEach(([colorKey, config]) => {
    ...
    const darkSteps = 4
    const lightSteps = 5
    const centerLightness = Color(config.centerColor).lightness()

    const lightnessStep = (config.lightest.lightness - centerLightness) / lightSteps
